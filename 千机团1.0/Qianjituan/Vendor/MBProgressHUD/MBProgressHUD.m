...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
    ...
    else if (mode == MBProgressHUDModeImageLoading){
        ...
        checkAnimating = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self
                                                        selector:@selector(checkAnimatingState) userInfo:nil repeats:YES];
        
        [indicator removeFromSuperview];
        
        UIImageView *loadingView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 25, 25)];
        
        NSMutableArray *arr = [[NSMutableArray alloc] init];
        
        for (NSInteger i = 1; i < 9; i++) {
            
            UIImage *temp = [UIImage imageNamed:[NSString stringWithFormat:@"progress_%d", i]];
        ...
        loadingView.animationRepeatCount = 99999;
        
        loadingView.animationDuration = 0.2;

        [loadingView startAnimating];
        
        self.indicator = loadingView;
...
//曾必兴 2014-06-30 用于检查前后台切换 动画停止
- (void)checkAnimatingState {
    
    if ([indicator isKindOfClass:[UIImageView class]]) {
        
        UIImageView *temp = indicator;
...
- (void)layoutSubviews {
    ...
    if (mode == MBProgressHUDModeImageLoading&&[self.labelText length] > 0) {
        
        //图片加载模式
        totalSize.height -= labelF.size.height;
        
        totalSize.width += indicatorF.size.width;
        
        CGRect boxRect = CGRectMake(roundf((bounds.size.width - totalSize.width) / 2) + self.xOffset,
                                    roundf((bounds.size.height - totalSize.height) / 2) + self.yOffset, totalSize.width, totalSize.height);
        
        CGFloat yPos = roundf(((bounds.size.height - totalSize.height) / 2)) + margin + yOffset;
        
        CGFloat xPos = margin *2/ 3;
        
        indicatorF.origin.y = yPos;
        
        indicatorF.origin.x = boxRect.origin.x + xPos;
        
        indicator.frame = indicatorF;
        
        labelF.origin.x = indicatorF.origin.x + indicator.frame.size.width + xPos;
        
        labelF.origin.y = yPos + (indicatorF.size.height - labelF.size.height)/2;
