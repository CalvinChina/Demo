...
//
//==============================================================================
@implementation UIImage (PisenKit)
// 旋转
...
- (UIImage*)psk_resizedImageToFitInSize:(CGSize)boundingSize scaleIfSmaller:(BOOL)scale {
    ...
    return [self psk_resizedImageToSize:dstSize];
}
...
- (UIImage *)psk_stretchImageWithPoint:(CGPoint)point {
    return [self psk_stretchImageWithEdgeInset:UIEdgeInsetsMake(point.y, point.x, point.y, point.x)];
}
...
+ (void)psk_addScreenBlurEffect {
    ...
    UIGraphicsEndImageContext();
    imageView.image = [screenImage psk_blurryImageWithBlurLevel:0.1];
