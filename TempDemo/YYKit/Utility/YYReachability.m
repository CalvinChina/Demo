...
@property (nonatomic,strong) NSDictionary * dictTelephonyStatus;
@property (nonatomic, assign) NSInteger telephonyStatus;
...
- (void)createTheTelephonyNetwork
{
    _networkInfo = [CTTelephonyNetworkInfo new];
    _dictTelephonyStatus = @{CTRadioAccessTechnologyGPRS : @(YYReachabilityWWANStatus2G),  // 2.5G   171Kbps
...
- (YYReachabilityWWANStatus)wwanStatus {
    ...
    NSNumber *num = _dictTelephonyStatus[status];
...
- (void)setTheNotifyBlock:(void (^)(YYReachability *reachability))notifyBlock {
    _notifyBlock = [notifyBlock copy];
    [self setTheScheduledState:(_notifyBlock != nil)];
}


- (void)setTheScheduledState:(BOOL)scheduled {
    if (_scheduled == scheduled) return;
    _scheduled = scheduled;
    if (scheduled) {
        if (NSFoundationVersionNumber >= NSFoundationVersionNumber_iOS_7_0) {
            [self createTheTelephonyNetwork];
        }
