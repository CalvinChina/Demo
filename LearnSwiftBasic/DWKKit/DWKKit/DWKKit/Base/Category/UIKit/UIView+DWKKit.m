...
- (void)makeBorderLine {
    [self makeBorderWithColor:DWKConfigDataInstance.defaultBorderColor borderWidth:1];
}
...
- (void)resetFontSize {
    [self resetFontSizeByXibWidth:DWKConfigDataInstance.xibWidth];
}
...
- (void)resetConstraint {
    [self resetConstraintByXibWidth:DWKConfigDataInstance.xibWidth];
}
...
- (void)_addTapWithTouches:(NSUInteger)numberOfTouches
    ...
    UITapGestureRecognizer *gesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(_handleAction:)];
...
- (void)setTapBlock:(void (^)(void))object {
